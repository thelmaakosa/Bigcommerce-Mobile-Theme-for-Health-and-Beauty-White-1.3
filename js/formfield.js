var FormField = {
	...
	'GetFieldByPrivateId': function (formId, privateId) {
	...
	'GetFieldByLabel': function (formId, label) {
	...
	'GetSelectorChoosePrefix': function (field) {
		var dd = FormField.GetFieldDefinitionData(field);
		if (!dd) {
			return'';
		}
	...
	'GetFieldPrivateId': function (field) {
		var dd = FormField.GetFieldDefinitionData(field);
		if (!dd) {
			return'';
		}
	...
	'GetFieldDefinitionData': function (field) {
	...
	'GetFieldDefinitionTag': function (field) {
		var dd = FormField.GetFieldDefinitionData(field);
		if (!dd) {
			return false;
		}
	...
	'GetValues': function (formIdx, privateOnly) {
		var values = [];
		...
		for (var i = formIdx.length; i--;) {
			...
			$('.FormFieldFormId[value=' + fieldFormId + ']').parent('dd, div.value').find('.FormField').each(function () {
				if (privateOnly && FormField.GetFieldPrivateId(this) == '') {
					return;
				}
				for (var i = 0; i < formIdx.length; i++) {
					values[values.length] = {'field': $(this), 'fieldId': FormField.GetFieldId(this), 'formId': fieldFormId, 'privateId': FormField.GetFieldPrivateId(this), 'label': FormField.GetLabel(this), 'value': FormField.GetValue(this)};
				}
			});
		}
		return values;
	},
	'SetValue': function (fieldId, val, options) {
		var field = FormField.GetField(fieldId);
		if (!field) {
			return false;
		}
		...
		switch (FormField.GetFieldType(field).toLowerCase()) {
			...
			case'checkboxselect':
				...
				$('.FormFieldOption', field).each(function () {
					var checked = false;
	...
	'SetValueByIndex': function (fieldId, indexes, checked) {
		var field = FormField.GetField(fieldId);
		if (!field) {
			return false;
		}
	...
	'GetOptions': function (fieldId, selectedOnly) {
		var field = FormField.GetField(fieldId);
		if (!field) {
			return false;
		}
	...
	'SetRequired': function (fieldId, status) {
		var field = FormField.GetField(fieldId);
	...
	'BindEvent': function (field, event, func, args) {
		field = FormField.GetField(field);
	...
	'UnBindEvent': function (field, event, func) {
		field = FormField.GetField(field);
		...
		if (rtn.msg == '') {
			switch (type) {
				...
				case'datechooser':
					...
					jQuery.each(thisValBits, function (i) {
						thisValBits[i] = parseInt(thisValBits[i], 10);
					});
					jQuery.each(limitFromBits, function (i) {
						limitFromBits[i] = parseInt(limitFromBits[i], 10);
					});
					jQuery.each(limitToBits, function (i) {
						limitToBits[i] = parseInt(limitToBits[i], 10);
					});
...
var FormFieldEvent = {
	'SingleSelectPopulateStates': function (event) {
		...
		if (countryName == '') {
			var options = {'display': 'option'};
			FormField.SetValue(stateId, '', options);
			return;
		}
		$.uniform.restore("#FormField_" + stateId);
		$.ajax({'global': false, 'url': 'remote.php', 'type': 'post', 'data': {'w': 'getStates', 'countryName': countryName}, 'success': function (data) {
			if ($('status', data).text() == '0') {
				return;
			}
			var states = [];
			$('options option', data).each(function () {
